angular.module('schemaCrawler', ['ngResource', 'clientUtilities'])
  .config(['$httpProvider', function($httpProvider) {
    /**
     * Intercepting responses
     */
    var interceptor = ['$window', '$location', '$q', 'jsonClient', 'base64', function($window, $location, $q, jsonClient, base64) {
      /**
 ...
 */
  .factory('jsonSchema', function($resource, $interpolate, $q, $window, $http, $location, jsonClient, base64) {
     ...
     * method recuses through the schema properties
     */
    ...
    apiClient.compileLink = function(root, fullPathData, data, pathParts, link) {
      ...
      if (angular.isArray(fullPathData[seg])) {  //If path points to an array,
        // Loop over all the elements
        for(var n in fullPathData[seg]) {
          // Dereference the data and the link
          var d = angular.copy(fullPathData[seg][n])
            , aLink = angular.copy(link);
          // Flatten/aggregate all the data at a single level deep literal
          angular.extend(fullPathData, fullPathData[seg][n]);
          // Re-curse into the array item, in case it is an object,
          // it may have properties to re-curse into
          apiClient.compileLink(root, fullPathData, d, angular.copy(pathParts), aLink);
        }
      }
      // Otherwise if the path points to an object, then
      if (!angular.isArray(fullPathData[seg]) && angular.isObject(fullPathData[seg])) {
        /// Dereference the data and the link
        var d = angular.copy(fullPathData[seg])
          , lLink = angular.copy(link);
        // Flatten/aggregate all the data at a single level deep literal
        angular.extend(fullPathData, fullPathData[seg]);
        // Recurse into object in case it has sub-properties
        apiClient.compileLink(root, fullPathData, d, angular.copy(pathParts), lLink);
      }
    };
    /**
     ...
     */
    apiClient.buildClient = function(url) { // This method does not need a schema  to traverse a link, it uses a url
      var def = $q.defer();
    ...
    apiClient.resourceURLTraverse = function(url, defaults, methods, method, payload, target, mime) {
      var deferred = $q.defer();
      if (target === "data") {
        // When the schema specifying data target,
        // then use the http method, and do not try to load a schema
        ...
        $resource(url, defaults, methods)[method](payload, function(response, headersFunc) {
          ...
          if (target !== 'nofollow' && target !== 'refresh') { // Don't change the url for refresh and nofollow links
            $location.url(url);
            apiClient.url = url;
          }
          deferred.resolve(response);
 ...
 *
 */
  .run(['$rootScope', '$location', 'jsonClient', function($rootScope, $location, jsonClient) {
    /**
